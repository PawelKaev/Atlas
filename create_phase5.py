...
fn test_plato_mode() {
    ...
    // Уменьшаем стабильность для возможности улучшения
    ...
    println!("Plato mode: {} iterations, {} events", 
             result.iterations, result.events.len());
    
    for event in &result.events {
        match event {
            PlatoEvent::SynthesisCompleted { name, confidence, .. } => {
                println!("  Synthesized: {} (confidence: {:.2})", name, confidence);
            }
            _ => {}
        }
    }
...
fn test_architect_mode() {
    ...
    println!("Architect result: {} (success: {})", result.message, result.success);
...
fn test_plato_empty_machine() {
    ...
    println!("Plato mode on empty machine: {} iterations", result.iterations);
}
'''

with open('grammalang-core/tests/ontology/phase5_tests.rs', 'w', encoding='utf-8') as f:
    f.write(content)
print("phase5_tests.rs fixed")
